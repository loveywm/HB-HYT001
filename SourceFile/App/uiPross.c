...
void uiProcMainDraw(BOOL Initialise,Menu_Parameter *Parameter)	/*¥˝ª˙ΩÁ√Êµƒœ‘ æ*/
{
	...
	for(ywm=0;ywm<3;ywm++)
	{
		if(uiTimeGetTickCount() - HB_Floor_Call_Buff[ywm].Call_Time_Count < 300)//3//3sƒ⁄œ‘ æ
		{
...
void HB_RELAY_Mode(void)
{
            ...
            {
                     HB_RELAY_Flag = 2;
		uiLcd_1616_ch(4,2,57,1);//œ‘ æœ¬ΩµÕº±Í
		DelayMs(1000);
            ...
            {
                    HB_RELAY_Flag = 1;
		 uiLcd_1616_ch(3,2,57,1);//œ‘ æ…œ…˝Õº±Í
		 DelayMs(1000);
            }
            else//Õ£÷π
            {
	...
	while(1)
	{
		...
		{	
		 ...
		 if(uiTimeGetTickCount() -Time_tmp >150)//1500ms∏¸–¬“ª¥Œ÷ÿ¡ø÷µ
			...
			//ªÒ»°µÁ—πµƒAD÷µ
			ad_temp = ADC_Filter_V();
			//¡¨–¯1.5s*40=60s∑÷÷”◊Û”“√ª”–ºÏ≤‚µΩµÁ—π ±£¨æÕ±£¥Êµ±«∞¬•≤„ ˝÷µ
