...
void AdcSystick10000Routine2(void)
{
	 static byte Counter =1;
    
    	while(!ADC_GetFlagStatus(ADC2, ADC_FLAG_EOC));//等待转换结束
    	*RegisterPointerBlock[Counter] = ADC_GetConversionValue(ADC2);

    	//if (++Counter > 3) Counter = 0;
    
    	ADC_RegularChannelConfig(ADC2,1,1,ADC_SampleTime_239Cycles5);//设置规则序列通道以及采样周期

    	ADC_SoftwareStartConvCmd(ADC2, ENABLE);//使能指定的 ADC2 的软件转换启动功能
...
void InitializeAdc2(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1;
        ...
        ADC_StartCalibration(ADC2);//开始指定 ADC2 的校准状态

        while(ADC_GetCalibrationStatus(ADC2));//等待校 AD 准结束

        ADC_SoftwareStartConvCmd(ADC2, ENABLE);//使能指定的 ADC2 的软件转换启动功能

        System.Device.Adc.Register2 = Register;
