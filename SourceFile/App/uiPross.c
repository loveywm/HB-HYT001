...
void uiProcMainDraw(BOOL Initialise,Menu_Parameter *Parameter)	/*¥˝ª˙ΩÁ√Êµƒœ‘ æ*/
{
	...
	if (Initialise)
	{
		...
		uiLcdMediumString("00",0+2,12,1);//œ‘ æ°∞ƒø±Í¬•≤„°± ˝◊÷
		
		//uiLcd_1212_ch(UISTR_WEIGHT,4,4,3);//œ‘ æ°∞µı÷ÿ£∫°±
		uiLcd_1616_ch(0,2+4,0,2);//œ‘ æ°∞÷ÿ¡ø°±
		...
		uiLcdMediumString("000",3,12,0);//’‚∏ˆ ˝÷µÃÓµı÷ÿ∞Ÿ∑÷±»
		...
		//uiLcd_1616_ch(8,0,2,3);//œ‘ æ  ∫ÙΩ–¬•≤„
		uiLcdMediumString(":",0,6,0);//√∞∫≈
	...
	//ºŸ»Á—≠ª∑≥¨π˝◊Ó¥Û¬•≤„£¨Àµ√˜µ±«∞÷µ“—æ≠‘⁄◊Ó¥Û¬•≤„…œ√Ê¡À
	//ƒ«√¥æÕ“™’“µΩ◊Ó¥Û¬•≤„£¨»ª∫Ûœ‘ æ
	if(ywm == App.Max_floor)
	{
		for(ywm=0;ywm<App.Max_floor;ywm++)
		{
			...
			{
				zuidalouceng = ywm+1;//º«¬º◊Ó∫Û”––ß±Íº«µƒ¬•≤„æÕ «◊Ó¥Û≤„
			}
		}
		//if(zuidalouceng==){}
...
static u8  Handle_Weight(Menu_Parameter *Parameter)
{
	...
	{
		uiLcdLineErase8x16(2,0,3,0);//Ω´œ»«∞◊¥Ã¨≤¡≥˝
		//Ã· æ‘§æØ
		...
		if(ad_temp >(App.Weight.Rated_weight*App.Weight.Alarm_weight/100))//≥¨π˝±®æØ÷µ
		{
			...
			fuck_count++;
			if(fuck_count >=2)//¡¨–¯¡Ω¥ŒæÕ∂œø™
			{
				Buzzer_Beep(3,100);//±®æØ»˝…˘
...
void Show_Param(void)
{
	...
	while(1)
	{
		 ...
		 if(uiTimeGetTickCount() -Time_tmp >100)//1000ms∏¸–¬“ª¥Œ
               	 {
			//µ√µΩ÷ÿ¡øµƒAD÷µ
...
//	u8 bRet;
	...
	switch(pUiMenuItem->nMenuStr)
	{
		...
		case UISTR_MENU_PINGCHENG_QUANSHAN://∆Ω≤„±Í∂®…æ≥˝
			...
			//Encoder_Demarcate_Init();
			break;
		case UISTR_MENU_PINGCHENG_SET_UI_ZIDONG://∆Ω≤„–ﬁ’˝ΩÁ√Ê
			Encoder_Demarcate_Init();
			break;
		//case UISTR_MENU_PINGCHENG_ZIDONGBUCANG://◊‘∂Ø≤π≥•
			//Auto_Encoder_Demarcate();
			//break;
		case UISTR_MENU_PINGCHENG_JISUAN_UP_BUCANG://º∆À„…œ…˝≤π≥•
			Auto_Fuck_Encoder(2);
			break;
		case UISTR_MENU_PINGCHENG_JISUAN_DOWN_BUCANG://º∆À„œ¬Ωµ≤π≥•
			Auto_Fuck_Encoder(3);
			break;
		case UISTR_MENU_PINGCHENG_JISUAN_OTHER_BUCANG://º∆À„ºıÀŸ≤π≥•
			Auto_Fuck_Encoder(1);
			break;
...
u8  uiProcKey(u8 nKey,Menu_Parameter *Parameter)
{
	...
	else if (nKey == UIKEY_OK)//ƒø±Í¬•≤„∆Ù∂Ø
	{
		...
		else if(Target_F > App.Max_floor)
		{
			...
			DelayMs(1200);
			Target_F =0;
			uiLcdLineErase8x16(0,8,17,0);//Ω´œ»«∞◊¥Ã¨≤¡≥˝
			
		}
		else
		{
			if(floor_tmp[Target_F-1].floor_flag == 0)//≈≈≥˝√ª”–±Í∂®µƒ¬•≤„
			{
				//uiLcd_1212_ch(UISTR_AIM_LOU_CHENG,0,66,2);
				//uiLcd_1212_ch(UISTR_AIM_LOU_CHENG+3,0,66+12*2,1);
				//uiLcd_1212_ch(UISTR_UI_GUODA,0,66+12*3,2);
...
void uiProcMain(void)
{
	...
	Time_tmp3 = Time_tmp2;
	Time_tmp4 = Time_tmp2;
	Floor_CurrentCount_pre =Floor_CurrentCount;//œ˚≥˝ø™ª˙ ±µƒπ‚±Í…¡À∏
	while(1)
	{
		...
		{	
			...
			{
				Time_tmp4 = uiTimeGetTickCount();//Ãÿ ‚∑µªÿ÷µ¥¶¿Ì
			}
		
		}
		uiProcProtocl();//¬•≤„∫ÙΩ–Ω” ‹¥¶¿Ì
		
		 if(uiTimeGetTickCount() -Time_tmp >100)//1000ms∏¸–¬“ª¥Œ÷ÿ¡ø÷µ
               	 {
                       	 //“‘∫Û√ø∏Ù2s÷”…œ¥´“ª¥Œ
                       	if(App.Weight.weight_display_change_flg ==1)
			{
			...
			//ªÒ»°µÁ—πµƒAD÷µ
			ad_temp = ADC_Filter_V();
			//¡¨–¯1.0s*4=4s◊Û”“√ª”–ºÏ≤‚µΩµÁ—π ±£¨æÕ±£¥Êµ±«∞¬•≤„ ˝÷µ
		...
		//Handle_Master(&Parameter);
		 ...
		 if(uiTimeGetTickCount() -Time_tmp2 >30)//300msªÒ»°“ª¥Œ±Í∂®±‡¬Î÷µ
               	 {
		...
		if(uiTimeGetTickCount() -Time_tmp3 >80)//800ms∏¸–¬“ª¥Œƒø±Í≤„…¡À∏
               	 {
		...
		if(HB_Start_Flag == 1)
		{
			//∞¥º¸∞¥œ¬6√Î∫Û£¨»Áπ˚∑∂Œß√ª≥¨π˝∞¥œ¬ «µƒ’˝∏∫10 ±£¨æÕ«Â¡„
