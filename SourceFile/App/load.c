...
u8 Load_Set_Calibration_tmp(u8 calibration)
{
	...
	if(calibration == 1)
	{
		uiLcd_1212_ch(UISTR_ZAIZHONG_SET_ZHONGLIANGZHILING,0,4,4);
		//uiLcd_1212_ch(UISTR_INPUT_LOAD,0,4+14*4,8);
		uiLcdDecimal(0,LOAD_ROW, LOAD_COL,0,1);
		uiLcdSmallString("Kg", 3, 4, 0);
	}
	if(calibration == 2)
	{
...
*/	
		...
		if(nKey == UIKEY_OK)
		{
			if(calibration == 1)
			{
				App.Weight.calibrate_one_suffer = App.Weight.Empty_weight;
