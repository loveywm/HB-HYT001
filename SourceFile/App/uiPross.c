...
void uiProcMainDraw(BOOL Initialise,Menu_Parameter *Parameter)	/*¥˝ª˙ΩÁ√Êµƒœ‘ æ*/
{
	...
	if (Initialise)
	{
		...
		uiLcd_1212_ch(UISTR_CURRENT_LOU_CHENG,0+2,4,2);//œ‘ æ°∞µ±«∞¬•≤„°±
		uiLcd_1212_ch(UISTR_CURRENT_LOU_CHENG+3,0+2,4+12*2,1);//œ‘ æ°∞µ±«∞¬•≤„°±

		
		uiLcd_1212_ch(UISTR_AIM_LOU_CHENG,0+2,76,2);//œ‘ æ°∞ƒø±Í¬•≤„°±
		uiLcd_1212_ch(UISTR_AIM_LOU_CHENG+3,0+2,76+12*2,1);//œ‘ æ°∞µ±«∞¬•≤„°

		
		uiLcdMediumString("00",0+1,5,0);//œ‘ æ°∞µ±«∞¬•≤„°± ˝◊÷
		uiLcdMediumString("11",0+1,14,1);//œ‘ æ°∞ƒø±Í¬•≤„°± ˝◊÷
		...
		uiLcdMediumString(":",1+1,4,0);//√∞∫≈
		
		uiLcdMediumString("0.00",1+1,5,0);//’‚∏ˆ ˝÷µÃÓµı÷ÿ µ ± ˝æ›
	...
	{
		if(Floor_CurrentCount < floor_tmp[ywm].floor_count)
		{
			if(ywm == 0)
			{
				uiLcdDecimal(1,0+1,5,0,2);
			}
			else
			{
				uiLcdDecimal(ywm,0+1,5,0,2);
			}
	...
	for(ywm=0;ywm<3;ywm++)
	{
		if(uiTimeGetTickCount() - HB_Floor_Call_Buff[ywm].Call_Time_Count < 1000)//10sƒ⁄œ‘ æ
		{
		...
		else
		{
			...
			uiLcdLineErase8x16(2+1,8+(ywm*3),8+(ywm*3)+3,0);//Ω´œ»«∞◊¥Ã¨≤¡≥˝
		}
...
static u8  Handle_Weight(Menu_Parameter *Parameter)
{
	...
	//ªÒ»°÷ÿ¡øµƒAD÷µ
	App.Weight_Value = ADC_Filter();
	//printf("App.Weight_Tmp===%d\r\n",App.Weight_Tmp);
			
	if(App.Weight.weight_clear_ad_value_sign == 0)//Œ™’˝
	{
		if(App.Weight_Value >= App.Weight.weight_clear_ad_value)
	         {
	                  ad_temp=App.Weight_Value-App.Weight.weight_clear_ad_value;
	         }
	         else
	         {
	               ad_temp=App.Weight.weight_clear_ad_value-App.Weight_Value;//œ‘ æ∏∫
	...
	{
		//uiLcdLineErase8x16(2,10,16,0);//Ω´œ»«∞◊¥Ã¨≤¡≥˝
		//Ã· æ‘§æØ
...
/*
	u8 tmp;
	for(tmp=0;tmp<App.Max_floor;tmp++)
	{
		if(tmp == call_floor)
		{
			WTV_Voice(ONE_FLAG);
...
void uiProcMain(void)
{
	...
	while(1)
	{
		...
		{	
		 ...
		 if(uiTimeGetTickCount() -Time_tmp >100)//1500ms∏¸–¬“ª¥Œ÷ÿ¡ø÷µ
               	 {
			...
			//uiLcdDecimal(ADC_Filter_V(),0,0,0,5);

			Handle_Voltage();

			
			//uiLcdDecimal(xxxx,3,12,0,4);
		...
		uiLcdDecimal(Floor_CurrentCount,2,11,0,5);
