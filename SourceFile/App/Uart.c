...
void HB_Send_Floor(Floor_Data   *floor)
{
        ...
        sum+=Ready_One_Byte_To_Send(floor->floor_num);//高位在后3


        sum+=Ready_One_Byte_To_Send(floor->floor_count);//低位在前4
        sum+=Ready_One_Byte_To_Send((floor->floor_count)>>8);//高位在后5
        sum+=Ready_One_Byte_To_Send((floor->floor_count)>>16);//6
        sum+=Ready_One_Byte_To_Send((floor->floor_count)>>24);//7
...
void HB_Send_Mode_Auto(u8 floor_count)
{
        ...
        TransmitByte2(PROTOCOL_HEAD_2);
        //命令字
        sum+=Ready_One_Byte_To_Send(CMD_LEVEL_MODE_AUTO);
        //数据长度
        sum+=Ready_One_Byte_To_Send(CMD_CURRENT_COUNT_DATA_LEN-3);
          //平层楼层数
        sum+=Ready_One_Byte_To_Send(floor_count);//低位在前
        //校验和
        TransmitByte2(sum);
        //尾
        TransmitByte2(PROTOCOL_TAIL_1);
...
void HB_Send_CMD_DATA4(u8 cmd,u32 data4)
{
        ...
        sum+=Ready_One_Byte_To_Send(cmd);
        //数据长度
        sum+=Ready_One_Byte_To_Send(4);
          //32位数据
        sum+=Ready_One_Byte_To_Send(data4);//低位在前
        sum+=Ready_One_Byte_To_Send(data4>>8);//高位在后
        sum+=Ready_One_Byte_To_Send(data4>>16);
        sum+=Ready_One_Byte_To_Send(data4>>24);
        //校验和
        TransmitByte2(sum);
        //尾
        TransmitByte2(PROTOCOL_TAIL_1);
...
void HB_Send_CMD_DATA1(u8 cmd,u8 data1)
{
        ...
        sum+=Ready_One_Byte_To_Send(cmd);
        //数据长度
        sum+=Ready_One_Byte_To_Send(1);
          //8位数据
        sum+=Ready_One_Byte_To_Send(data1);//低位在前
